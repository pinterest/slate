...
export default class StatsboardChart extends React.Component {
  ...
  componentDidMount = () => {
    this.fetchData(this.props.statsboardUrl).then((data) => {
      this.setState({
        dataLoaded: true,
        response: data,
      });
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.statsboardUrl !== this.props.statsboardUrl) {
      this.fetchData(this.props.statsboardUrl).then((data) => {
        this.setState({
          dataLoaded: true,
          response: data,
  ...
  formatResponse(response) {
    ...
    for (var responseGroup of response) {
      ...
      if (tags.length > 0) {
        seriesName = tags[0];
        if (metric.includes("logging.audit")) {
          if (metric.includes("lost")) {
            seriesName += " - lost";
          } else {
            seriesName += " - corrupted";
          }
        }
      }
  ...
  render() {
    if (this.state.response !== null && this.state.dataLoaded) {
      let [formattedResponse, seriesSet] = this.formatResponse(
        this.state.response
      );
